...
impl PathConfig {
    ...
    fn arith_chip(&self) -> ArithChip<Fp> {
...
impl PathChip {
    pub fn configure(
        ...
        PathConfig {
            ...
            advices,
            poseidon_config,
            arith_config: ArithChip::configure(
                meta,
                utility_advices[0],
                utility_advices[1],
                utility_advices[2],
    ...
    fn decompose_value(value: &Fp) -> Vec<Fp> {
        ...
        let mut bits: Vec<Fp> = bits[..HEIGHT].iter().map(|x| Fp::from(*x)).collect();
        bits.resize(HEIGHT, Fp::from(0));
        bits
    }
    ...
    fn check_membership(
        ...
        path: [Value<Fp>; HEIGHT],
    ) -> Result<AssignedCell<Fp, Fp>, plonk::Error> {
        // Witness values
        let (bits, path, zero) = layouter.assign_region(
            || "witness",
            |mut region| {
                let bits = pos.value().map(Self::decompose_value).transpose_vec(HEIGHT);
        ...
        let arith_chip = self.config.arith_chip();
...
mod tests {
    use super::*;
    use darkfi_sdk::crypto::smt2::{MemoryStorage, Poseidon, SparseMerkleTree};
    use halo2_proofs::{
    ...
    impl Circuit<Fp> for TestCircuit {
        ...
        fn synthesize(
            ...
            // Witness values
            let (root, leaf, pos, one) = layouter.assign_region(
                || "witness",
                |mut region| {
                    ...
                    Ok((root, leaf, pos, one))
                },
            )?;

            let is_valid = path_chip.check_membership(&mut layouter, root, leaf, pos, self.path)?;
    ...
    fn test_smt_circuit() {
        ...
        let mut witnessed_path = [Value::unknown(); HEIGHT];
        for (i, p) in path.path.into_iter().enumerate() {
            witnessed_path[i] = Value::known(p);
        }
